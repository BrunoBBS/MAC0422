    ...
    while (1)
    {
        if (proc_count > 0)
        {
            ...
            for (int i = 0; i < cpu_cnt; i++)
            {
                ...
                {
                    if (globals.extra)
                        printf("with the ENDED process %s\n", running_p[i]->name);
                    // frees semaphore
                ...
                // Checks time for each cpu
                for (int j = 0; j < cpu_cnt; j++)
                {
                    if (t_now - startt[i] >= QUANTUM)
                    {
                        if (globals.extra)
                            printf("[RR ] CPU %d changing processes from %s to %s\n",
                                   j,p_pointer->proc->name, p_pointer->next->proc->name);
